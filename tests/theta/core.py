...
def get_uniswap_theta_grid(tx_file: str,
    ...
    df: pd.DataFrame = pd.read_csv(tx_file, dtype={'total_liquidity': 'float','sqrtPriceX96': "float"})
...
def get_avg_fee(from_dir: str,
    ...
    for day_file in files:
        df: pd.DataFrame = pd.read_csv(os.path.join(from_dir, day_file), dtype={'total_liquidity': 'float','sqrtPriceX96': "float",'current_tick': "float"})
        date_str = re.findall("\\d{4}-\\d{2}-\\d{2}", day_file)[0]
        amount_column = "amount" + ("0" if pool_info.is_0_base else "1")
        df = df.loc[df.tx_type == "SWAP"]
        df = df[[amount_column, "current_tick", "total_liquidity","sqrtPriceX96", "tick_lower", "tick_upper"]]
        df = df.rename(columns={amount_column: "amount"})
        df = df.astype({'amount': 'float64'})


        lower_tick = quote_price_to_tick_nobase(price * lower_rate, pool_info.token0_decimal, pool_info.token1_decimal)
        upper_tick = quote_price_to_tick_nobase(price * upper_rate, pool_info.token0_decimal, pool_info.token1_decimal)

        lower_sqrt_x96 = int(df.sqrtPriceX96.min()) # 当天最低价格
        ...
        upper_sqrt_x96 = int(df.sqrtPriceX96.max()) # 当天最高价格  
        ...
        fee_rate = __calc_one_tick(df, lower_tick, pool_info, upper_tick)
        # print(lower_tick,lower_price,upper_tick,upper_price)
    ...
    print(result_df['date'].max())

    if pool_info.is_0_base:
        price = 1 / price
    lower = price * lower_rate
    upper = price * upper_rate

    selected_rows = result_df[(result_df['upper_price'] <= upper) & (result_df['lower_price'] >= lower) 
                            & (result_df['date'] >= result_df['date'].max() - pd.Timedelta(days=30))]
    result_df['selected'] = result_df.isin(selected_rows).astype(int)['upper_price']
    result_df.to_csv('result1.csv', index=False)
    print(result_df)

    avg_fee_rate = selected_rows['fee_rate'].mean()
...
# def fee_rate_avg(lower_rate: Decimal,
#                 upper_rate: Decimal,
#                 price: Decimal,
#                 df:pd.DataFrame):
#     lower = price * lower_rate
...
#     selected_rows = df[(df['upper_price'] <= upper) & (df['lower_price'] >= lower)]
...
def get_tick_fee(df: pd.DataFrame, pool_info: PoolInfo, tick_lower: int, tick_upper: int, liquidity: int) -> Decimal:
    ...
    if len(ndf.index) > 0:
        ...
        # ndf["amount"] = ndf["amount"].astype(float)
