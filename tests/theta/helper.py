...
def sqrtPriceX96_to_quote_price(sqrt_price: int, token_0_decimal, token_1_decimal, is_token0_base: bool):
    decimal_price = _x96_to_decimal(sqrt_price) ** 2
    pool_price = decimal_price * Decimal(10 ** (token_0_decimal - token_1_decimal))
    return Decimal(1 / pool_price) if is_token0_base else pool_price


def quote_price_to_tick_nobase(price: Decimal, token_0_decimal: int, token_1_decimal: int) -> int:
    # quote price->add decimal pool price->sqrt_price ->ticker
    sqrt_price = quote_price_to_sqrt_nobase(price, token_0_decimal, token_1_decimal)
    tick = sqrt_price_to_tick(sqrt_price)
    return tick


def quote_price_to_sqrt_nobase(price: Decimal, token_0_decimal: int, token_1_decimal: int) -> int:
    # quote price->add decimal pool price->sqrt_price ->ticker
